...
__attribute__((sdx_kernel("AES_Encrypt", 0))) void AES_Encrypt(byte input[16], byte output[16], word key[4]);
...
void SubBytes(byte state[4][4]) {
    VITIS_LOOP_17_1: for (int i = 0; i < 4; i++) {
        VITIS_LOOP_18_2: for (int j = 0; j < 4; j++) {
...
void MixColumns(byte state[4][4]) {
    byte Tmp, Tm, t;
    VITIS_LOOP_56_1: for (int i = 0; i < 4; i++) {
        t = state[0][i];
...
void AddRoundKey(byte state[4][4], word w[4 * (10 + 1)], int round) {
    VITIS_LOOP_75_1: for (int i = 0; i < 4; i++) {
        VITIS_LOOP_76_2: for (int j = 0; j < 4; j++) {
            state[j][i] ^= (w[round * 4 + i] >> (8 * (3 - j))) & 0xff;
        }
    }
}

__attribute__((sdx_kernel("AES_Encrypt", 0))) void AES_Encrypt(byte input[16], byte output[16], word key[4]) {
#line 23 "/home/rrk307/home_hls/python_c_hlsc/other_application/cryptography/run_hls.tcl"
#pragma HLSDIRECTIVE TOP name=AES_Encrypt
# 82 "aes.cpp"
    ...
    VITIS_LOOP_90_1: for (int round = 1; round < 10; round++) {
        SubBytes(state);
        ShiftRows(state);
        MixColumns(state);
    ...
    VITIS_LOOP_101_2: for (int i = 0; i < 4; i++) {
        VITIS_LOOP_102_3: for (int j = 0; j < 4; j++) {
...
void test_AES() {
    ...
    VITIS_LOOP_116_1: for (int i = 0; i < 16; i++) {
        printf("%02x ", output[i]);
    }
    printf("\n");
}
