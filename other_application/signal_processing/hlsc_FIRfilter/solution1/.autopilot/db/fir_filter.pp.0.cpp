...
__attribute__((sdx_kernel("fir_filter", 0))) void fir_filter(hls::stream<data_t>& input, hls::stream<data_t>& output, coeff_t coeffs[FILTER_ORDER]) {
#line 23 "/home/rrk307/home_hls/python_c_hlsc/other_application/signal_processing/run_hls.tcl"
...
void test_fir_filter() {
    ...
    VITIS_LOOP_50_1: for (int i = 0; i < NUM_SAMPLES; i++) {
        input.write(i);
    }
    ...
    VITIS_LOOP_59_2: for (int i = 0; i < NUM_SAMPLES; i++) {
        if (!output.empty()) {
