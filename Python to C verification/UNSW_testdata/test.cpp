...
int main() {
    ...
    if (sizeof(test_data) / sizeof(test_data[0]) != 82332 || sizeof(test_data[0]) / sizeof(float) != 43) {
        std::cerr << "Error: Incorrect test data dimensions" << std::endl;
        return 1;
    }

    if (sizeof(test_labels) / sizeof(int) != 82332) {
        std::cerr << "Error: Incorrect test labels dimensions" << std::endl;
        return 1;
    }

    for (int i = 0; i < 82332; ++i) {
        std::vector<float> input(test_data[i], test_data[i] + 43);
        try {
    ...
    float accuracy = static_cast<float>(correct_predictions) / 82332 * 100;
